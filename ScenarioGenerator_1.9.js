...
(() => {
  ...
  function q() {
    return {
      ...
      eraId: s,
      step: 1,
      title: "Khai cục Tàn Minh của ta",
  ...
  async function _e() {
    ...
    if (a.length > 42e3)
      throw new Error(
        `Thiết lập hoàn chỉnh của nhân vật có mặt tại hiện trường tổng cộng ${a.length} ký tự, đã vượt quá ngân sách tạo ổn định. Vui lòng giảm bớt nhân vật có mặt tại màn mở đầu; các nhân vật không có mặt vẫn được bảo lưu trong phần Nhân vật khái lãm.`,
      );
  ...
  function tt({ preserveScroll: e = !1 } = {}) {
    ...
    const t = (e && L.querySelector(".sg-content")?.scrollTop) || 0,
      ...
      a = [Ie, Ke, Ze, et];
    if (
      ((L.innerHTML = `<div class="sg-shell"><header class="sg-head"><div class="sg-brand"><span class="sg-seal">☘</span><span><b>Trình tạo mở đầu</b><small>Sùng Trinh năm thứ bảy tháng bảy · DLC Thân phận</small></span></div><div class="sg-head-actions"><button type="button" class="sg-api-trigger" data-action="open-api-settings" title="Dùng chung cấu hình API với Vạn Tượng Sinh Thành Khí"><span>⚙ API</span><small>${E($e())}</small></button><button class="sg-close" data-action="close" aria-label="Đóng">×</button></div></header><main class="sg-main"><nav class="sg-steps">${["Ta là ai", "Ta quen ai", "Lời dẫn câu chuyện", "Tạo DLC"].map((e, t) => `<button class="sg-step ${M.step === t + 1 ? "on" : ""}" data-step="${t + 1}"><i>${t + 1}</i><span>${e}</span></button>`).join("")}</nav><div class="sg-content">${a[M.step - 1]()}</div></main><footer class="sg-footer"><div class="sg-status ${D}">${E(J || W || "Bản nháp tự động lưu cục bộ.")}</div><div class="sg-actions"><button class="sg-btn" data-action="reset">Tạo mới</button>${M.step > 1 ? '<button class="sg-btn" data-action="previous">Bước trước</button>' : ""}${M.step < 4 ? '<button class="sg-btn primary" data-action="next">Bước tiếp</button>' : ""}</div></footer></div>`),
      Ue(),
      e)
    ) {
